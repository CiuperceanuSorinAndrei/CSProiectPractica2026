...
# --- accumulated_volume_m3: depth (mm) * area (m^2) -> volume (m^3) --------------------

def test_accumulated_volume_basic():
    # 10 mm (= 0.01 m) over 1,000,000 m^2 = 10,000 m^3
...
def test_accumulated_volume_one_mm_over_one_m2_is_one_litre():
    # 1 L/m^2 over 1 m^2 = 1 L = 0.001 m^3 (verifies the mm->m factor)
...
# --- fill_percentage: accumulated volume as a percentage of the maximum volume ------------------------

def test_fill_percentage_known_value():
    # 100 mm over 1 km^2 (1e6 m^2) = 100,000 m^3; compared to 1 million m^3 -> 10%
...
def test_fill_percentage_matches_suhaia_hand_calc():
    # Suhaia: ~9.99 km^2, 18 million m^3. 50 mm -> 0.05 * 9.99e6 / 18e6 * 100 = 2.775%
...
def test_fill_percentage_can_exceed_100():
    # An extreme episode can exceed capacity; we do not cap it.
...
def test_fill_percentage_none_when_capacity_unknown(max_vol):
    # Without a valid capacity, we cannot report a percentage -> None (the card stays without a percentage).
    assert ReservoirFillEstimator.fill_percentage(100.0, 1_000_000.0, max_vol) is None


# --- fill_percentage_for: convenient variant on the ReservoirLoader input ----------------
...
def test_fill_percentage_for_none_reservoir():
    # City/circle mode (no reservoir) does not produce a percentage.
...
def test_fill_percentage_for_missing_keys_returns_none():
    # Reservoir without a known maximum volume -> None (get(...) implicit 0.0 -> invalid capacity).
    assert ReservoirFillEstimator.fill_percentage_for(100.0, {"surface_area_m2": 1_000_000.0}) is None


# --- Integration with the real shapefile (skipped if data is missing) -------------------------

@pytest.mark.skipif(not os.path.exists(SHAPEFILE), reason="Reservoir shapefile is not available")
def test_loader_attaches_volume_and_area_for_known_reservoir():
    ...
    assert "Suhaia" in reservoirs, "The reference reservoir Suhaia should exist in the shapefile"

    suhaia = reservoirs["Suhaia"]
    # Known maximum volume of Suhaia: 18 million m^3.
    assert suhaia["vol_mil_m3"] == pytest.approx(18.0)
    assert suhaia["max_volume_m3"] == pytest.approx(18_000_000.0)
    # Surface area ~9.99 km^2 (derived from the projected geometry, in m^2).
    assert suhaia["surface_area_m2"] == pytest.approx(9.99e6, rel=0.01)

    # End-to-end contract: 50 mm accumulated -> ~2.78% of capacity.
